...
def move_mouse_to_image(img, confidence=0.9, duration=1.0):
    try:
        ...
        button_end_point = (np.random.uniform(button_to_press[0]-offset, button_to_press[0]+offset), np.random.uniform(button_to_press[1]-offset, button_to_press[1]+offset))
...
def human_click(button='left', trained=False):
    if trained:
        human_reaction_time = (0.190, 0.215)
    else:
    ...
    try:
        ...
        while True:
            ...
            if is_image_on_screen(cast_img, confidence=0.70):
                ...
                while True:
                    if is_image_on_screen(reel_img, confidence=0.80, grayscale=True):
